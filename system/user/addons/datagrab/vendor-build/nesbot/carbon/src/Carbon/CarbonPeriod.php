...
class CarbonPeriod implements Iterator, Countable, JsonSerializable
{
    ...
    public function __construct(...$arguments)
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null && (\is_string($argument) && \preg_match('/^(-?\\d(\\d(?![\\/-])|[^\\d\\/-]([\\/-])?)*|P[T0-9].*|(?:\\h*\\d+(?:\\.\\d+)?\\h*[a-z]+)+)$/i', $argument) || $argument instanceof DateInterval || $argument instanceof Closure) && ($parsed = @CarbonInterval::make($argument))) {
    ...
    {
        return \is_string($callable) && \substr($callable, 0, 2) === 'is' && (\method_exists($this->dateClass, $callable) || [$this->dateClass, 'hasMacro']($callable));
    }
    /**
