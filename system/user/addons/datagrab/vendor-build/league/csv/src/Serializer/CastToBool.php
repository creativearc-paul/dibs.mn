...
final class CastToBool implements TypeCasting
{
    ...
    public function toVariable(mixed $value) : ?bool
    {
        ...
        return match (\true) {
            Type::True->equals($this->type) && \true !== $returnValue && !$this->isNullable, Type::False->equals($this->type) && \false !== $returnValue && !$this->isNullable => throw TypeCastingFailed::dueToInvalidValue(match (\true) {
                null === $value => 'null',
                '' === $value => 'empty string',
                default => $value,
