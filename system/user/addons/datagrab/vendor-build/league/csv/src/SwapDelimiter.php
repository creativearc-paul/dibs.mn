...
final class SwapDelimiter extends php_user_filter
{
    ...
    public static function appendTo(mixed $stream, string $inputDelimiter, string $delimiter) : mixed
    {
        ...
        $filter = \stream_filter_append($stream, self::getFiltername(), params: ['mb_separator' => $inputDelimiter, 'separator' => $delimiter, 'mode' => \str_contains(\stream_get_meta_data($stream)['mode'], 'r') ? self::MODE_READ : self::MODE_WRITE]);
    ...
    public static function prependTo(mixed $stream, string $inputDelimiter, string $delimiter) : mixed
    {
        ...
        \set_error_handler(fn(int $errno, string $errstr, string $errfile, int $errline) => \true);
        $filter = \stream_filter_append($stream, $filtername, params: ['mb_separator' => $inputDelimiter, 'separator' => $delimiter, 'mode' => \str_contains(\stream_get_meta_data($stream)['mode'], 'r') ? self::MODE_READ : self::MODE_WRITE]);
        \restore_error_handler();
