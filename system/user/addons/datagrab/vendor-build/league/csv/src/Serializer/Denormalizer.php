...
final class Denormalizer
{
    ...
    {
        return $this->mapRecord?->afterMappingMethods($this->class) ?? AfterMapping::from($this->class)?->mapRecord->afterMappingMethods($this->class) ?? [];
    }
    /**
     * @param array<string> $propertyNames
    ...
    private function autoDiscoverPropertySetter(ReflectionMethod|ReflectionProperty $accessor, array $propertyNames, array $methodNames) : ?PropertySetter
    {
        ...
        return match (\true) {
            ...
            default => new PropertySetter($accessor, $offset, $this->resolveTypeCasting($reflectionProperty), $this->mapRecord?->convertEmptyStringToNull ?? self::$convertEmptyStringToNull, $this->mapRecord?->trimFieldValueBeforeCasting ?? \false),
        };
    }
    /**
     * @param array<string> $propertyNames
     ...
     * @throws MappingFailed
     */
    private function getTypeCasting(string $typeCaster, ReflectionProperty|ReflectionParameter $reflectionProperty, array $options) : TypeCasting
    {
        try {
            /** @var TypeCasting $cast */
            $cast = match (\str_starts_with($typeCaster, CallbackCasting::class . '@')) {
                \true => new CallbackCasting($reflectionProperty, \substr($typeCaster, \strlen(CallbackCasting::class))),
