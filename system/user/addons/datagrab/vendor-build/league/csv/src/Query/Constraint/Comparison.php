...
namespace BoldMinded\DataGrab\Dependency\League\Csv\Query\Constraint;

use BoldMinded\DataGrab\Dependency\League\Csv\Query\QueryException;
...
enum Comparison : string
{
    ...
    {
        match ($this) {
            self::Between, self::NotBetween => match (\true) {
                !is_array($reference), !array_is_list($reference), 2 !== count($reference) => throw new QueryException('The value used for comparison with the `' . $this->name . '` operator must be an list containing 2 values, the minimum and maximum values.'),
                default => \true,
            },
            self::In, self::NotIn => match (\true) {
                !is_array($reference) => throw new QueryException('The value used for comparison with the `' . $this->name . '` operator must be an array.'),
                default => \true,
            },
            self::Regexp, self::NotRegexp => match (\true) {
                !is_string($reference), '' === $reference, \false === @\preg_match($reference, '') => throw new QueryException('The value used for comparison with the `' . $this->name . '` operator must be a valid regular expression pattern string.'),
                default => \true,
            },
            self::Contains, self::NotContain, self::StartsWith, self::EndsWith => match (\true) {
                !is_string($reference), '' === $reference => throw new QueryException('The value used for comparison with the `' . $this->name . '` operator must be a non empty string.'),
                default => \true,
            },
            self::Equals, self::NotEquals, self::GreaterThanOrEqual, self::GreaterThan, self::LesserThanOrEqual, self::LesserThan => \true,
        };
    }
}
