...
final class TypeCastingFailed extends RuntimeException implements SerializationFailed
{
    ...
    private static function format(string $message, ?TypeCastingInfo $info = null) : string
    {
        ...
        $target = TypeCastingTargetType::MethodFirstArgument === $info->targetType ? 'the first argument `' . $target . '` of the method `' . $className . $info->targetMethodName . '()`' : 'the property `' . $className . $target . '`';
        $source = $info->source;
