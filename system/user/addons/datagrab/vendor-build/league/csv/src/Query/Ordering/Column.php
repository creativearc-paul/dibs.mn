...
namespace BoldMinded\DataGrab\Dependency\League\Csv\Query\Ordering;

use ArrayIterator;
use Closure;
use Iterator;
use BoldMinded\DataGrab\Dependency\League\Csv\Query\QueryException;
use BoldMinded\DataGrab\Dependency\League\Csv\Query\Row;
use BoldMinded\DataGrab\Dependency\League\Csv\Query\Sort;
...
final class Column implements Sort
{
     ...
     */
    private function __construct(public readonly string $direction, public readonly string|int $column, public readonly Closure $callback)
    {
    }
    /**
     ...
     */
    public static function sortOn(string|int $column, string|int $direction, Closure|callable|null $callback = null) : self
    {
        $operator = match (\true) {
