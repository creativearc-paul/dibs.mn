...
enum Bom : string
{
    ...
    {
        return self::tryFromSequence($sequence) ?? throw new ValueError('No BOM sequence could be found on the given sequence.');
    }
    public static function tryFromSequence(mixed $sequence) : ?self
    {
        $str = match (\true) {
            $sequence instanceof SplFileObject, $sequence instanceof Stream => self::getContents($sequence, 4, 0),
            \is_resource($sequence) => \stream_get_contents($sequence, 4, 0),
            $sequence instanceof AbstractCsv => $sequence->getInputBOM(),
            $sequence instanceof Stringable, \is_string($sequence) => \substr((string) $sequence, 0, 4),
            default => $sequence,
        };
    ...
    {
        return self::tryFromEncoding($name) ?? throw new ValueError('Unknown or unsupported BOM name `' . $name . '`.');
    }
    /**
