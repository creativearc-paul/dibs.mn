...
class PhpRedisConnector implements Connector
{
    ...
    public function connect(array $config, array $options)
    {
        $connector = function () use($config, $options) {
            return $this->createClient(\array_merge($config, $options, Arr::pull($config, 'options', [])));
        };
    ...
    {
        return $this->formatHost($server) . ':' . $server['port'] . '?' . Arr::query(Arr::only($server, ['database', 'password', 'prefix', 'read_timeout']));
    }
    /**
