...
class Datagrab_model extends CI_Model
{
    ...
    public function setup(AbstractDataType $dataType, array $settings = [], bool $shouldProduce = false): Datagrab_model
    {
        ...
        // Remove when support for 7.4 is dropped, and Laravel and Carbon dependencies are updated.
    ...
    private function prepareUrlTitle(string $urlTitle): string
    {
        ...
        $urlTitle = preg_replace('/_{1,}/', '-', $urlTitle);
     ...
     * @return array
     */
    private function prepareMetaData($entryData): array
    {
        $metaData = [];
     ...
     * @param int|float $filedir
     * @param bool      $fetchUrl
     * @return string
     */
    public function getFile(string $filename, $filedir = 1, bool $fetchUrl = false): string
    {
        return (new File($filename, $filedir, $fetchUrl, $this->logger))->fetch();
    }
